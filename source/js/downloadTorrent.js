...
function decodeBytes(bytes, encoding) {
  ...
  for (i = 0; i < n; i++) {chars[i] = enc.charAt(bytes.charCodeAt(i));}
  return chars.join('');
}
...
function prepareTorrentRequest(request, callback) {
  if (!request || (!request.torrent && !request.url)) {
    callback(null);
    return;
  }

  if (request.torrent && request.data) {
    callback(request);
    return;
  }
  ...
  getFile(request.url, function (file) {
    parseTorrent(file, function (torrent) {
      ...
      encodeFile(file, function (data) {
        callback(Object.assign({}, request, {
          torrent : torrent,
          data    : data
        }));
      });
...
function renderTorrent(request) {
  ...
  var paused = jQuery('#downloadPauseTorrent');
  ...
  if (typeof request.torrent.info.files === 'undefined') {
  ...
  jQuery('#save').click(function () {
    var message = {
      ...
      blacklist : getNotSelectedFilesIds()
    };
...
chrome.runtime.sendMessage({method: 'get-torrent-info', page: 'torrent'}, function (request) {
  prepareTorrentRequest(request, function (preparedRequest) {
    if (!preparedRequest) {
      jQuery('#torrentName').text('Unable to load torrent metadata.');
      jQuery('#save').prop('disabled', true);
      return;
    }

    renderTorrent(preparedRequest);
  });
});
